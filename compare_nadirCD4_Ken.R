...
length(unique(IDs1[!is.na(IDs1)])) #24 have same count, 25 different
...
length(unique(IDs2[!is.na(IDs2)])) #22 have same date, 27 different
...
length(unique(IDs4[!is.na(IDs4)])) #40 have same min CD4 percent, 9 different
IDs3=test_ken$ID.x[as.Date(test_ken$nadir_cd4_percent_date,"%Y-%m-%d")!=test_ken$nadir_CD4percent_date]
length(unique(IDs3[!is.na(IDs3)])) #36 have same min CD4 percent date, 13 have different
# Different ones are 116 180 194* 195* 202 95*  97*  98*  100* and also 101 112 126 201 for date
