...
length(unique(d4$SID)) #seems like too many subjects? Because there are CD4 measures for some controls in 2015/2016!

# Use Martha's demog spreadsheet for ages/scan dates etc - note the 4 year excel fix, always be aware!
...
#count(cd4_data_all_HIV$ID)$x[count(cd4_data_all_HIV$ID)$freq<20] #These all less than 20 visit dates: 100 104 105 107 136 165 190 194 195 208 209 214 215 216 95  97  98
...
# Identify 17 children who were older than 20 weeks at their first avail CD4 measure
test$ID[test$age_at_first_lab>20] #100 104 105 107 136 165 190 194 195 208 209 214 215 216 95  97  98 

# Identify children who were younger than 9 years at their first avail CD4 measure
test$ID[test$age_at_last_lab<9] #153 183 188 214 215 216

# Check Ken's nadir CD4 against mine
# compare_nadirCD4_Ken.R

age7IDs=test[!is.na(test$Scan.date.7yrs),c('ID')]
age9IDs=test[!is.na(test$Scan.date.9yrs.Skyra),c('ID')]

needed=intersect(age7IDs,missing) #"100" "104" "105" "107" "214" "215" "216" "95"  "98" "101" "112" "126" "180"
needed=intersect(age9IDs,missing) #"105" "136" "165" "190" "194" "195" "208" "98" "101" "112" "126" "180" phri